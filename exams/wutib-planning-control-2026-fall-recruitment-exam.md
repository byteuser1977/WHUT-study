...
```text
方案一（上）：期望速度 → [下位机] → [电机] → 实际速度
方案二（下）：期望速度 → [控制器] → [下位机] → [电机] → 实际速度
...
```text
┌─────────────────────────────────────────────────────────────────┐
│ 文件层   路径文件 / yaml 文件参数设置                            │
│          实车移动采集路径点 ← GPS/INS                            │
├─────────────────────────────────────────────────────────────────┤
│ 传感器   上位机 → 凸包处理 / 障碍物 / 以车长半宽裁剪 /           │
│          可行性路段 / 滤波（取值）/ 地面平整度                    │
├─────────────────────────────────────────────────────────────────┤
│ 规划层   实时规划器 → 收束可行路段 / 障碍物节点剪枝 /            │
│          DP 规划路线 → 路径平滑 → 路径输出                       │
├─────────────────────────────────────────────────────────────────┤
│ 控制层   控制器 → 纵向控制（路面情况、预期速度）/                 │
│                 横向控制（Stanley 算法、转角输出）                │
├─────────────────────────────────────────────────────────────────┤
│ 车机层   下位机控制 → 电机控制 → 实际速度                        │
└─────────────────────────────────────────────────────────────────┘
```
